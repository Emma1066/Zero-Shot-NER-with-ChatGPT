...
def copy_rationale_to_parse(args, data, data_parse):
    for item_parse, item in tqdm(zip(data_parse, data), desc="copy rationale"):
        item_parse["label_rationale"] = item["label_rationale"]
    return data_parse

def main(args):
    demo_ra_data = load_data(args.demo_ra_data_path)
    demo_parse_data = load_data(args.demo_parse_data_path)
    
    demo_parse_ra_data = copy_rationale_to_parse(args, demo_ra_data, demo_parse_data)
    save_data(args.demo_parse_ra_data_path, demo_parse_ra_data)

    print(f"Save data_parse_ra to {args.demo_parse_ra_data_path}")
